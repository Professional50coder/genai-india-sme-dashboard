"""
╔══════════════════════════════════════════════════════════════════╗
║   GenAI India SME — Market Entry Strategy Dashboard             ║
║   Real-Time Data + Interactive Slide Viewer                     ║
╚══════════════════════════════════════════════════════════════════╝
"""
...
# ──────────────────────────────────────────────────────────────────
    ...
    with right:
        ...
        fig2.update_yaxes(title_text="SME Count (Million)", secondary_y=False)
        fig2.update_yaxes(title_text="% Digitally Active", secondary_y=True, gridcolor="transparent")
...
# ──────────────────────────────────────────────────────────────────
    ...
    with left:
        ...
        fig.update_layout(**PLOTLY_LAYOUT, height=280,
            xaxis=dict(range=[0, 45], title="% of respondents"),
    ...
    with right:
        ...
        fig2.update_layout(**PLOTLY_LAYOUT, height=280,
            xaxis=dict(range=[0, 100], title="% SMEs want this feature"),
            yaxis=dict(autorange="reversed"),
        )
        st.plotly_chart(fig2, use_container_width=True)
...
# ──────────────────────────────────────────────────────────────────
    ...
    with tab1:
        ...
        fig.update_yaxes(title_text="ARR (₹ Crore)", secondary_y=False)
        fig.update_yaxes(title_text="Paying SMEs", secondary_y=True, gridcolor="transparent")
        st.plotly_chart(fig, use_container_width=True)
...
# ──────────────────────────────────────────────────────────────────
    ...
    for col, (phase, period, title, color, points) in zip([col1,col2,col3], phases):
        bullet_html = "".join([f'<li style="margin:8px 0;font-size:13px;color:#CBD5E1;">{p}</li>' for p in points])
        with col:
            st.markdown(f"""
    ...
    for col, (name, price, period, color, feats, tag) in zip([p1, p2, p3], tiers):
        feat_html = "".join([f'<div style="padding:5px 0;border-bottom:1px solid #1E3A4F;font-size:13px;color:#CBD5E1;">✓ {f}</div>' for f in feats])
        with col:
            st.markdown(f"""
...
# ──────────────────────────────────────────────────────────────────
    ...
    with left_c:
        ...
        fig_mrr.update_layout(**PLOTLY_LAYOUT, height=240,
            xaxis=dict(showticklabels=False, gridcolor="transparent"),
    ...
    fig_city.update_layout(**PLOTLY_LAYOUT, height=280,
        xaxis=dict(title="City", gridcolor="transparent"),
        yaxis=dict(title="New Signups (This Month)"),
    )
    ...
    for icon, ts, event_type, detail in events:
        st.markdown(f"""
        <div style="display:flex;gap:14px;align-items:flex-start;padding:8px 0;border-bottom:1px solid #1E3A4F;">
